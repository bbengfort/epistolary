...
func (suite *epistolaryTestSuite) TestJWKS() {
...
func (suite *epistolaryTestSuite) TestOpenIDConfiguration() {
	require := suite.Require()

	// Create a basic HTTP request rather than use the Epistolary client to ensure the
	...
	require.Equal("http://localhost:8000/", openid.Issuer)
	require.Equal("http://localhost:8000/.well-known/jwks.json", openid.JWKSURI)
}

func (suite *epistolaryTestSuite) TestSecurityTxt() {
	require := suite.Require()
